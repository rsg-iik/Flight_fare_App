...
app = Flask(__name__)
model = pickle.load(open("rfModelPrediction.pkl", "rb")) # loading the model file from the storage
...
def predict():
    ...
    #    scalar = pickle.load(open('sandardScalar.pkl', 'rb'))
...
def from_postman():
    if (request.method == 'POST'):
        ...
        # Date_of_Journey
        date_dep = request.json["Dep_Time"]
        Journey_Day = int(pd.to_datetime(date_dep, format="%Y-%m-%dT%H:%M").day)
        Journey_Month = int(pd.to_datetime(date_dep, format ="%Y-%m-%dT%H:%M").month)
        # print("Journey Date : ",Journey_day, Journey_month)
        ...
        # Arrival
        date_arr = request.json["Arrival_Time"]
        Arrival_hour = int(pd.to_datetime(date_arr, format ="%Y-%m-%dT%H:%M").hour)
        Arrival_min = int(pd.to_datetime(date_arr, format ="%Y-%m-%dT%H:%M").minute)
        # print("Arrival : ", Arrival_hour, Arrival_min)
        ...
        # Total Stops
        Total_Stops = int(request.json["stops"])
        # print(Total_stops)
